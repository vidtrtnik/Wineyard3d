...
class Wineyard3D
{
  constructor(canvas, resx, resy, pp)
  {
    ...
    renderer = new wy3d_Renderer(this.resolution_x, this.resolution_y, this.pp);
    this.RESOURCES = [];
    this.SCENES = [];
  ...
  {
    const newScene = new wy3d_Scene(name, gl, renderer);
  ...
  initWebGL(canvas)
  {
    ...
    try
    {
      tmp_gl = canvas.getContext("webgl2", {antialias: true})// || canvas.getContext("webgl", {antialias: false});
    }
    catch (e)
    {
  ...
  renderLogoScene()
  {
    ...
    const m_logo = new wy3d_Model(this, m_res_logo.getRes());
    const t_logo = new wy3d_Texture(this, t_res_logo.getRes());

    this.logoScene = this.addScene("LogoScene", gl, renderer);
